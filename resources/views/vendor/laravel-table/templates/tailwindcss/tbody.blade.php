<tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
    @if ($table->getPaginator()->isEmpty())
        <tr {{ html_classes($table->getTrClasses()) }}>
            <td {{ html_classes($table->getTdClasses(), 'text-center', 'px-3') }}{{ html_attributes($table->getColumnsCount() > 1 ? ['colspan' => $table->getColumnsCount()] : null) }}
                scope="row">
    ...
    @else
        @foreach ($table->getPaginator() as $model)
            <tr
                {{ html_classes('text-gray-700 dark:text-gray-400', $model->conditionnal_classes, $model->disabled_classes) }}>
